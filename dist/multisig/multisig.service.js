...
let MultisigService = MultisigService_1 = class MultisigService {
    walletRepository;
    signatureRepository;
    signatureCollectionWorkflow;
    recoveryService;
    dataSource;
    logger = new common_1.Logger(MultisigService_1.name);
    SIGNATURE_EXPIRY_HOURS = 24;
    constructor(walletRepository, signatureRepository, signatureCollectionWorkflow, recoveryService, dataSource) {
        this.walletRepository = walletRepository;
        this.signatureRepository = signatureRepository;
        this.signatureCollectionWorkflow = signatureCollectionWorkflow;
        this.recoveryService = recoveryService;
        this.dataSource = dataSource;
    }
    async createWallet(createWalletDto, creatorId) {
    ...
    async signTransaction(signTransactionDto, signerId) {
        ...
        if (wallet.status === multisig_wallet_entity_1.WalletStatus.LOCKED || wallet.status === multisig_wallet_entity_1.WalletStatus.TERMINATED) {
            throw new common_1.BadRequestException('Wallet is not available for transactions');
        }
        ...
        if (existingSignature) {
            throw new common_1.ConflictException('Signature already exists for this transaction');
        }
    ...
    async revokeSignature(revokeDto, signerId) {
        ...
        if (!signature) {
            throw new common_1.NotFoundException('Pending signature not found');
        }
        if (!signature.canRevoke) {
            throw new common_1.BadRequestException('Signature cannot be revoked');
        }
        signature.status = signature_entity_1.SignatureStatus.REVOKED;
    ...
    async executeTransaction(executeDto, executorId) {
        ...
        if (signatures.length === 0) {
            throw new common_1.NotFoundException('No collected signatures found for this transaction');
        }
        const wallet = signatures[0].wallet;
        if (!wallet.signers.includes(executorId)) {
            throw new common_1.BadRequestException('Executor is not authorized for this wallet');
        }
        const requiredSignatures = wallet.requiredSignatures;
        if (signatures.length < requiredSignatures) {
            throw new common_1.BadRequestException(`Insufficient signatures. Required: ${requiredSignatures}, Collected: ${signatures.length}`);
        }
        try {
            const executionResult = await this.executeMultisigTransaction(wallet.address, executeDto.transactionHash, signatures.map(s => s.signature), executeDto.executionData);
            const updatedSignatures = await Promise.all(signatures.map(async (signature) => {
                signature.status = signature_entity_1.SignatureStatus.EXECUTED;
            ...
            throw new common_1.BadRequestException('Transaction execution failed');
        }
    }
    async getTransactionStatus(transactionHash) {
        ...
        const collectedSignatures = signatures.filter(s => s.status === signature_entity_1.SignatureStatus.COLLECTED);
        const executedSignatures = signatures.filter(s => s.status === signature_entity_1.SignatureStatus.EXECUTED);
        let status = 'pending';
    ...
    async queryMultisigData(queryDto) {
    ...
    async cleanupExpiredSignatures() {
        ...
        for (const signature of expiredSignatures) {
            signature.status = signature_entity_1.SignatureStatus.EXPIRED;
    ...
    validateWalletCreation(createWalletDto) {
        if (createWalletDto.signers.length < 2 || createWalletDto.signers.length > 15) {
            throw new common_1.BadRequestException('Number of signers must be between 2 and 15');
        }
        if (createWalletDto.threshold < 2 || createWalletDto.threshold > createWalletDto.signers.length) {
            throw new common_1.BadRequestException('Threshold must be between 2 and the number of signers');
        }
        if (createWalletDto.recoveryThreshold) {
            if (createWalletDto.recoveryThreshold < createWalletDto.threshold ||
                ...
                throw new common_1.BadRequestException('Recovery threshold must be between normal threshold and number of signers');
            }
        }
        const uniqueSigners = new Set(createWalletDto.signers);
        if (uniqueSigners.size !== createWalletDto.signers.length) {
            throw new common_1.BadRequestException('Signers must be unique');
        }
    }
    async generateMultisigAddress(signers, threshold) {
        return `multisig_${signers.join('_')}_${threshold}_${Date.now()}`;
    }
    async executeMultisigTransaction(walletAddress, transactionHash, signatures, executionData) {
...
], MultisigService.prototype, "cleanupExpiredSignatures", null);
exports.MultisigService = MultisigService = MultisigService_1 = __decorate([
